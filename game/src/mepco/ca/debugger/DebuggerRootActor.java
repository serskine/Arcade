...
public class DebuggerRootActor extends ArcadeRootActor {
    ...
    public static final int CONTROLS_AREA_HEIGHT = CONTROLLER_HEIGHT*2 + UNIT;
    ...
    public static final int TEXT_HEIGHT = UNIT;
    ...
    public void render(Graphics2D g) {
        ...
        final Dimension textSize = Draw.getTextSize(g, TEXT_HEIGHT, title);
        final int tX = (width-textSize.width)/2;
        final int tY = TEXT_HEIGHT;

        final Point pTitle = new Point(tX, tY);
        Draw.renderRetroText(g, pTitle, TEXT_HEIGHT, COLOR_ACCENT, COLOR_FOREGROUND, title);
    ...
    private void renderMonitorState(Graphics2D g, Point p, ArcadeMonitorState monitorState) {
        ...
        final int textHeight = UNIT;

        Dimension wSize = Draw.getTextSize(g, textHeight, wText);
        Dimension hSize = Draw.getTextSize(g, textHeight, hText);
        Dimension opSize = Draw.getTextSize(g, textHeight, opText);
        ...
        final int innerHeight = MONITOR_AREA_HEIGHT - wSize.height - opSize.height - hSize.height - textHeight - textHeight;
        final int pWidthY = p.y + (MONITOR_AREA_HEIGHT - innerHeight) / 2;
        final int pOpY = pWidthY + wSize.height + textHeight;
        final int pHeightY = pOpY + opSize.height + textHeight;
        ...
        Draw.renderRetroText(g, pWidth, textHeight, COLOR_ACCENT, COLOR_FOREGROUND, wText);
        Draw.renderRetroText(g, pOp, textHeight, COLOR_ACCENT, COLOR_FOREGROUND, opText);
        Draw.renderRetroText(g, pHeight, textHeight, COLOR_ACCENT, COLOR_FOREGROUND, hText);
